...
router.post('/add', upload.single('image'),async (req, res) => {
  try {
    ...
    const { Menu } = getModelsFromRequest(req);
...
router.put("/toggle/:id", async (req, res) => {
  ...
  try {
    ...
    io.emit("menuStatusChanged", {
      itemId: item._id,
      paused: item.paused,
      tenantId: tenantId, // Include tenantId in socket event
    });
...
// Get all menu items (automatically filtered by tenant DB)
