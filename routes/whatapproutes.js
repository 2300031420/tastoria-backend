...
// Get tenantId for WhatsApp orders (from env or default to 'tastoria')
const getWhatsAppTenantId = () => {
  const tenantId = process.env.WHATSAPP_CAFE_NAME?.toLowerCase() || process.env.WHATSAPP_TENANT_ID?.toLowerCase();
  // Normalize tenant ID (remove special chars)
  if (tenantId) {
    return tenantId.replace(/[^a-z0-9]/g, '');
  }
...
const getMenuByCategory = async () => {
  ...
  const { Menu } = getTenantModels(tenantId);
  const items = await Menu.find({ paused: false });
  const categories = {};
    ...
    switch (session.step) {
      ...
      case 'confirmPrompt': {
        if (text.includes('yes')) {
          const tenantId = getWhatsAppTenantId();
          const { Order } = getTenantModels(tenantId);
          const cafeDisplayName = tenantId === 'tastoria' ? 'Tastoria' : tenantId.charAt(0).toUpperCase() + tenantId.slice(1);
